...
STOCKS = dataset[['Issuer Name', 'Security Code']]
STOCKS = STOCKS[:500].values.tolist()
header = {'User': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:76.0) Gecko/20100101 Firefox/76.0'}
DATA = []
async def get_stock_data(session, bseid, fullname):
    global DATA
    ...
    async with session.get(url) as response:        
        ...
        try:
            ...
            x = {'Name':bseid,'FullName':fullname,'Rate':LTP,'Change':c,'PChange':pc}
            DATA.append(x)
...
async def get_top_10_movers():
    ...
    async with aiohttp.ClientSession() as session:
        tasks = []
        DATA = []
        for bseid, fullname in STOCKS:
            task = asyncio.ensure_future(get_stock_data(session, bseid, fullname))
            tasks.append(task)
